...
class staggered_grid_2D_simple(object): 
    def __init__(self, Lx, Ly, Nx, Ny, Eps=8.854e-3, Mu=1254.9):
        """sets up a grid with homogeneous material constants
            parameters:
                Lx,Ly - grid dimensions
                Nx,Ny - number of knots
            -> this class does not include ADEs (auxiliary differential
            equations) for materials with non-constant properties (e.g.
            metals)"""
...
def CalculationStep_simple(grid0, t, excitations):
    """does the next step of the calculation
           - uses leapfrog and a staggered grid
        parameters:
            dt - time step
        ...
        output:
            Ex1,Ey1 - E-field, time n + 1
            Hz1 - H-field, time n + 1.5
        -> no ADEs, only constant material properties
    """
    # create the new grid:
    ...
    def rhsE():
        ...
        grid1.Ex = grid0.Ex + grid0.dt / grid0.Eps * rotH_x
        grid1.Ey = grid0.Ey + grid0.dt / grid0.Eps[:-1, :-1] * rotH_y
    ...
    def rhsH():
        """Right hand side of the second part: H-field (Faraday's law)
           -> H - field is calculated at times (N+1.5)*dt on the half integer 
                    grid"""        
        #calculate components of rot E at points (i+.5)*dx and times (N+1.5)*dt:
        rotE_z1 = (grid1.Ey[:, 1:] - grid1.Ey[:, :-1]) # dEy/dx
